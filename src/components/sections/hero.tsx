"use client";

import Image from "next/image";

import { useI18n } from "@/i18n/provider";
import { TextureButton } from "@/components/ui/texture-button";

const HERO_BLUR_DATA_URL =
  "data:image/jpeg;base64,";

export function Hero() {
  const { messages } = useI18n();

  return (
    <section id="top" className="flex w-full flex-1 flex-col">
      <Image
        src="/images/hero-illustration.webp"
        alt={messages.hero.imageAlt}
        width={1920}
        height={822}
        priority
        quality={90}
        placeholder="blur"
        blurDataURL={HERO_BLUR_DATA_URL}
        sizes="100vw"
        className="aspect-8/3 w-full object-cover object-top select-none"
      />

      <div className="mx-auto grid w-full max-w-content gap-10 px-4 pt-8 pb-16 md:px-8 lg:grid-cols-[2fr_1fr] lg:items-start">
        <h1 className="max-w-xl font-pixel-circle font-medium tracking-tighter text-7xl">
          {messages.hero.title}
        </h1>

        <div className="flex max-w-fit flex-col gap-6 w-full pt-1">
          <p className="font-sans text-lg text-muted font-light tracking-tight w-full">
            {messages.hero.description}
          </p>

          <div className="flex flex-wrap items-center gap-3">
            <TextureButton asChild variant="primary" size="pill" className="w-auto">
              <a href="#contacto">{messages.hero.primaryCta}</a>
            </TextureButton>
            <TextureButton asChild variant="secondary" size="pill" className="w-auto">
              <a href="#servicios">{messages.hero.secondaryCta}</a>
            </TextureButton>
          </div>
        </div>
      </div>
    </section>
  );
}
